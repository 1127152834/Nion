...
void test("workspace thread history helpers define a fixed three-tab taxonomy", async () => {
  const source = await readFile(
    new URL("./history-tabs.ts", import.meta.url),
    "utf8",
  );

  assert.match(source, /export type WorkspaceThreadType = "general" \| "project" \| "bridge"/);
