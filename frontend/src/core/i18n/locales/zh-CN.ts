...
export const zhCN: Translations = {
  ...
  settings: {
    ...
    channels: {
      title: "通道",
      description: "管理通道凭证、接入模式、配对授权与运行状态。",
      workspace: {
        title: "单工作区",
        description:
          "Nion 使用单一应用工作区。通道授权会绑定到当前应用工作区，而不是让运营人员在多个工作区之间切换。",
      },
      platforms: {
        lark: "Lark",
        dingtalk: "钉钉",
      ...
      configuration: {
        title: "通道配置",
        description: "先填写凭证并验证连通性，再在同一界面处理配对和授权。",
      },
      labels: {
        enabled: "启用",
        disabled: "停用",
      ...
      hints: {
        pairingGuide:
          "连接成功只说明通道可达。下一步：请让用户先发送任意一条消息，然后在下方的“待处理配对请求”中批准它。",
      ...
      runtime: {
        statusTitle: "运行状态",
        statusDescription: "这张卡片展示运行时健康度和运营侧可见状态，应与正在运行的 connector 保持一致。",
      ...
      pairing: {
        sectionTitle: "配对与授权",
        sectionDescription: "生成临时配对码、批准进入的配对请求，并管理已授权用户。",
        code: {
          title: "配对码",
          description:
            "建议先让用户发送任意消息。`/pair 123456` 仍可作为手动兜底方案。",
          expireMinutes: "分钟后过期",
          generateAction: "生成",
          activeCode: "当前配对码",
          noCodeGenerated: "尚未生成配对码",
          copiedToast: "已复制配对命令",
          generatedToast: "配对码已生成：{code}",
          generateFailed: "生成配对码失败",
          expiresAtPrefix: "过期时间",
          slotHint: "生成后会在这里显示 6 位配对码",
        },
        pending: {
          title: "待处理配对请求",
          empty: "暂无待处理请求",
          approvedToast: "已批准并授权",
          rejectedToast: "已拒绝",
        },
      },
      authorization: {
        ...
        revokeConfirmTemplate: '确认撤销 "{name}" 的通道授权吗？该用户需要重新配对。',
        revokedToast: "授权已撤销",
      },
      session: {
        defaultsTitle: "会话默认值",
        defaultsDescription: "为该通道配置默认会话参数。只有显式填写的字段才会发送到运行时。",
    ...
    configSections: {
      ...
      sandbox: {
        ...
        strictMode: "严格模式",
        strictModeTipEn:
          "启用后将禁止 host 执行，并强制使用 AIO 沙箱提供方。该语义必须与 web 端聊天页的 sandbox/host toggle 保持一致。",
        strictModeTipZh:
          "启用后将禁止 host 执行，并强制使用 AIO 沙箱提供方。该语义必须与 web 端聊天页的 sandbox/host toggle 保持一致。",
        baseUrl: "Base URL",
