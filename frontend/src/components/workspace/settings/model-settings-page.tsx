...
export function ModelSettingsPage() {
  ...
  const [activeView, setActiveView] =
    useState<ModelSettingsChildView>("providers");
  const {
    draftConfig,
    validationErrors,
    validationWarnings,
  ...
  const models = useMemo(() => asArray(normalizedConfig.models), [normalizedConfig]);
      ...
      {isLoading ? (
        ...
        <div className="text-destructive text-sm">
          {error instanceof Error
            ? error.message
            : (m?.loadConfigFailed ?? "Failed to load config")}
        </div>
      ) : (
