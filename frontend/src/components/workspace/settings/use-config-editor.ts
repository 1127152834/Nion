...
function extractValidationErrors(
  ...
  return errors
    .map((item) => {
      ...
      const type =
        typeof source.type === "string" ? source.type : "validation_error";
      return {
        path: Array.isArray(source.path)
          ? source.path.map((v) => String(v))
          : [],
        message,
        type,
...
function extractValidationWarnings(
  detail: unknown,
): ConfigValidateWarningItem[] | null {
  if (!detail || typeof detail !== "object") {
    return null;
  }
  ...
  return warnings
    .map((item) => {
      ...
      const message = typeof source.message === "string" ? source.message : "";
      const type =
        typeof source.type === "string" ? source.type : "validation_warning";
      return {
        path: Array.isArray(source.path)
          ? source.path.map((v) => String(v))
          : [],
...
export function useConfigEditor(options: UseConfigEditorOptions = {}) {
  ...
  const { configData, runtimeStatus, isLoading, error, refetchConfig } =
    useConfigCenter();
