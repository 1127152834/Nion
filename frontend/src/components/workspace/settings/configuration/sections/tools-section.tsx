...
const TOOL_PRESETS: ToolPreset[] = [
  ...
  },
  {
    ...
    name: "ls",
    group: "file:read",
    use: "nion.sandbox.tools:ls_tool",
  ...
  },
  {
    ...
    name: "read_file",
    group: "file:read",
    use: "nion.sandbox.tools:read_file_tool",
    title: "Read File",
  ...
  },
  {
    ...
    name: "write_file",
    group: "file:write",
    use: "nion.sandbox.tools:write_file_tool",
    title: "Write File",
  ...
  },
  {
    ...
    name: "str_replace",
    group: "file:write",
    use: "nion.sandbox.tools:str_replace_tool",
...
export function ToolsSection({
  ...
  const customToolCount = useMemo(() => {
    return tools.filter((tool) => {
      ...
      const use = asString(tool.use).trim();
      return !TOOL_PRESETS.some(
        (preset) => name === preset.name || use === preset.use,
      );
    }).length;
  }, [tools]);
  ...
  const customInfo =
    customToolCount > 0
      ? section.customInfoTemplate.replaceAll(
          "{count}",
          String(customToolCount),
        )
      : "";
      ...
      <section className="space-y-3 rounded-lg border p-4">
        ...
        <div className="space-y-2">
          {TOOL_PRESETS.map((preset) => {
            ...
            return (
              <div className="rounded-md border px-3 py-2.5" key={preset.id}>
