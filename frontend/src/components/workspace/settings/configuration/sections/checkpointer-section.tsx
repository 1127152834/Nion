...
export function CheckpointerSection({
  ...
  const hasUnsupportedType =
    Boolean(rawType) && rawType !== "memory" && rawType !== "sqlite";
  const rawConnectionString = asString(checkpointer.connection_string);
  const connectionString =
    type === "sqlite" && !hasUnsupportedType ? rawConnectionString : "";

  const updateCheckpointer = (
    updates: Partial<Record<"type" | "connection_string", unknown>>,
  ) => {
  ...
  const connectionHint =
    type === "sqlite" ? copy.sqliteHint : copy.memoryHint;
