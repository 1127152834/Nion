...
from nion.skills.installer import resolve_skill_dir_from_archive_root
...
def test_resolve_skill_dir_ignores_macosx_wrapper(tmp_path: Path) -> None:
    ...
    assert resolve_skill_dir_from_archive_root(tmp_path) == tmp_path / "demo-skill"
...
def test_resolve_skill_dir_rejects_archive_with_only_metadata(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="Skill archive is empty"):
        resolve_skill_dir_from_archive_root(tmp_path)
