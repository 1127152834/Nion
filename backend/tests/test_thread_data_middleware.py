...
class TestThreadDataMiddleware:
    def test_before_agent_returns_paths_when_thread_id_present_in_context(self, tmp_path):
        ...
        assert result["thread_data"]["workspace_path"].endswith("threads/thread-123/user-data/workdir")
    ...
    def test_before_agent_uses_thread_id_from_configurable_when_context_is_none(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "nion.agents.middlewares.thread_data_middleware.get_config",
        ...
        assert result is not None
        assert result["thread_data"]["workspace_path"].endswith("threads/thread-from-config/user-data/workdir")
