...
def test_notebook_asset_move_updates_asset_directory(monkeypatch, tmp_path):
    monkeypatch.setenv("NION_HOME", str(tmp_path))
    reset_paths()
    ...
    with TestClient(create_app()) as client:
        ...
        moved = client.post(
            f"/api/notebook/assets/{asset_id}/move",
            json={"directory": "资料归档"},
        )

        assert moved.status_code == 200
        payload = moved.json()["asset"]
        assert payload["asset_id"] == asset_id
        assert payload["relative_path"] == "资料归档/report.html"
