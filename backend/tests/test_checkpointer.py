...
class TestGetCheckpointer:
    def test_returns_in_memory_saver_when_not_configured(self):
        ...
        with patch("nion.agents.checkpointer.provider.get_app_config", side_effect=FileNotFoundError):
            cp = get_checkpointer()
...
class TestClientCheckpointerFallback:
    def test_client_uses_config_checkpointer_when_none_provided(self):
        ...
        with (
            patch("nion.client.get_app_config", return_value=config_mock),
            patch("nion.client.create_agent", side_effect=fake_create_agent),
            patch("nion.client.create_chat_model", return_value=MagicMock()),
            patch("nion.client._build_middlewares", return_value=[]),
            patch("nion.client.apply_prompt_template", return_value=""),
            patch("nion.client.NionClient._get_tools", return_value=[]),
        ):
