...
class TestDeferredToolsPromptSection:
    ...
    def _mock_app_config(self, monkeypatch):
        ...
        from nion.config.tool_search_config import ToolSearchConfig

        mock_config = MagicMock()
        mock_config.tool_search = ToolSearchConfig()  # disabled by default
        monkeypatch.setattr("nion.config.get_app_config", lambda: mock_config)
...
class TestDeferredToolFilterMiddleware:
    ...
    def _ensure_middlewares_package(self):
        ...
        That file replaces nion.agents and nion.agents.middlewares with
        ...
        mock_keys = [
            ...
            "nion.agents.middlewares.deferred_tool_filter_middleware",
        ]
