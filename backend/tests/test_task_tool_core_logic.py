...
task_tool_module = importlib.import_module("nion.tools.builtins.task_tool")
...
def test_task_tool_emits_running_and_completed_events(monkeypatch):
    ...
    # task_tool lazily imports from nion.tools at call time, so patch that module-level function.
    monkeypatch.setattr("nion.tools.get_available_tools", get_available_tools)
...
def test_task_tool_passes_fork_active_skill_to_subagent_executor(monkeypatch):
    config = _make_subagent_config()
    captured = {}
    active_skill = {
        "name": "planner",
    ...
    output = task_tool_module.task_tool.func(
        runtime=_make_runtime(active_skill=active_skill),
        description="运行子任务",
        prompt="collect diagnostics",
        subagent_type="general-purpose",
    ...
    assert output == "Task Succeeded. Result: done"
    assert captured["executor_kwargs"]["active_skill"] == active_skill
