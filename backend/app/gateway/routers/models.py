...
class ModelConnectionTestRequest(BaseModel):
    ...
    model: str | None = Field(
        default=None,
        description="Provider model id. When omitted, the backend probes the provider model-list endpoint instead.",
    )
...
class ModelConnectionTestResponse(BaseModel):
    ...
    response_preview: str | None = Field(default=None, description="Short preview from model response when successful")


class ProviderModelsRequest(BaseModel):
    """Request model for fetching a provider model catalog."""
...
class ProviderModelOption(BaseModel):
    ...
    source: str = Field(default="provider", description="Metadata source: provider or models.dev")
...
class ProviderModelListUnsupportedError(Exception):
    """Raised when a provider does not expose a model listing API."""
...
async def _fetch_provider_models_anthropic(
    ...
    base = base.rstrip("/")
    candidate_urls: list[str] = [f"{base}/v1/models", f"{base}/models"]
...
def _merge_provider_models_with_models_dev(
    ...
    for item in provider_models:
        ...
        merged.append(ProviderModelOption(id=model_id, name=item.get("name") or None, source="provider"))
...
@router.post(
    ...
    description="Resolve a model's capabilities from models.dev.",
)
async def inspect_model_metadata(request: ModelMetadataRequest) -> ModelMetadataResponse:
    target_model_id = request.model.strip()
    provider_key = _guess_models_dev_provider_key(request.use.strip(), _strip_optional(request.api_base))
