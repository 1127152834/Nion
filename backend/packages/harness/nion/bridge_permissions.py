...
@dataclass
class BridgePermissionRequestRecord:
    id: str
    thread_id: str
    tool_name: str
...
def _store_path() -> Path:
    return get_paths().base_dir / "bridge_permissions.json"
...
def create_bridge_permission_request(
    *,
    thread_id: str,
    tool_name: str,
    tool_input: dict[str, Any],
    ...
    store = _read_store()
    record = BridgePermissionRequestRecord(
...
def resolve_bridge_permission_request(
    *,
    thread_id: str,
    permission_request_id: str,
    decision: PermissionDecision,
) -> BridgePermissionRequestRecord | None:
    store = _read_store()
    for item in store["requests"]:
        if item["id"] != permission_request_id or item["thread_id"] != thread_id:
            continue
        if item["status"] != "pending":
            return BridgePermissionRequestRecord(**item)
...
def get_bridge_permission_request(
    *,
    thread_id: str,
    permission_request_id: str,
) -> BridgePermissionRequestRecord | None:
...
def get_bridge_permission_profile(thread_id: str) -> str:
    store = _read_store()
    return store.get("thread_profiles", {}).get(thread_id, "default")


def consume_bridge_pending_allow(
    *,
    thread_id: str,
    tool_name: str,
    tool_input: dict[str, Any],
