...
from nion.config.paths import VIRTUAL_PATH_PREFIX, Paths, get_paths
...
def _normalize_mode(value: str | None) -> ExecutionMode:
    return "host" if value == "host" else "sandbox"
...
class RuntimeProfileRepository:
    def __init__(self, base_dir: str | Path | None = None):
        self._paths = Paths(base_dir) if base_dir is not None else get_paths()
    ...
    def _coerce_profile(raw: dict[str, Any]) -> RuntimeProfile:
        return RuntimeProfile(
            execution_mode=_normalize_mode(raw.get("execution_mode")),  # type: ignore[arg-type]
    ...
    def validate_host_workdir(path_value: str | None) -> str:
        ...
        if normalized is None:
            raise RuntimeProfileValidationError(
                "host_workdir is required when binding a host directory"
            )
        ...
        try:
            ...
            raise RuntimeProfileValidationError(
                f"host_workdir cannot be read: {normalized}"
            ) from exc
    ...
    def update(
        ...
        *,
        ...
        if existing_workdir is not None:
            if next_mode == "host":
                if next_workdir is None:
                    ...
                    raise RuntimeProfileValidationError(
                        "host_workdir is already bound to this thread"
                    )
            else:
                next_workdir = existing_workdir

        if next_mode == "host" and next_workdir is not None:
            next_workdir = self.validate_host_workdir(next_workdir)
        elif next_mode == "sandbox":
            next_workdir = existing_workdir
        ...
        if existing["locked"] and (
            ...
            raise RuntimeProfileLockedError(
                "Runtime profile is locked after first host-bound execution"
            )
