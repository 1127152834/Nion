"""Configuration storage using SQLite with optimistic versioning."""
...
class ConfigStoreNotInitializedError(Exception):
    """Raised when config store metadata is requested before initialization."""
...
class SQLiteConfigStore:
    """SQLite-backed configuration store with optimistic locking."""
    ...
    def _coerce_bootstrap_config(raw: Any) -> dict[str, Any]:
        ...
        if not isinstance(checkpointer, dict) or not isinstance(
            checkpointer.get("type"), str
        ):
            ...
            if raw_type not in ("memory", "sqlite"):
                config["checkpointer"] = deepcopy(DEFAULT_CHECKPOINTER_CONFIG)
            elif raw_type == "sqlite" and not checkpointer.get(
                "connection_string"
            ):
                config["checkpointer"] = deepcopy(DEFAULT_CHECKPOINTER_CONFIG)
    ...
    def _ensure_initialized(self) -> None:
        with self._connect() as conn:
            ...
            row = conn.execute(
                f"SELECT version FROM {self.TABLE_NAME} WHERE id = 1"
            ).fetchone()
            if row is not None:
                return
    ...
    def read(self) -> tuple[dict[str, Any], str, Path]:
        ...
        with self._connect() as conn:
            row = conn.execute(
                f"SELECT version, config_json FROM {self.TABLE_NAME} WHERE id = 1"
            ).fetchone()
    ...
    def write(self, config_dict: dict[str, Any], expected_version: str) -> str:
        ...
        with self._connect() as conn:
            ...
            serialized = yaml.safe_dump(
                config_dict, sort_keys=False, allow_unicode=True
            )
            conn.execute(
...
def resolve_config_db_path() -> Path:
    """Resolve the SQLite database path for the config center."""
...
def create_config_store(config_path: str | Path | None = None) -> ConfigStore:
    """Create the active config store backend."""
    if config_path is not None:
        raise ValueError("Legacy config_path loading is not supported")

    mode = os.getenv("NION_CONFIG_STORAGE", "auto").strip().lower()
    if mode == "file":
        raise ValueError(
            "NION_CONFIG_STORAGE=file is no longer supported; use auto or sqlite"
        )
