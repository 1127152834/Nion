...
def _normalize_tool_call_args(raw_args: object) -> tuple[dict, str | None]:
    """Normalize tool call args to a dict plus optional fallback key."""
...
def _hash_tool_calls(tool_calls: list[dict]) -> str:
    ...
    for tc in tool_calls:
        ...
        normalized.append(f"{name}:{_stable_tool_key(name, args, fallback_key)}")
...
class LoopDetectionMiddleware(AgentMiddleware[AgentState]):
    ...
    def _apply(self, state: AgentState, runtime: Runtime) -> dict | None:
        ...
        if warning:
            # Inject as HumanMessage to avoid provider-specific restrictions
            # around mid-conversation system messages.
