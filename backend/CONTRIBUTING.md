# Contributing to Nion Backend

Thank you for your interest in contributing to Nion! This document provides guidelines and instructions for contributing to the backend codebase.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/nion.git
   cd nion
   ```
...
```python
import pytest
from nion.models.factory import create_chat_model
...
1. Create tool in `packages/harness/nion/tools/builtins/` or `packages/harness/nion/community/`:

```python
# packages/harness/nion/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: nion.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/nion/agents/middlewares/`:

```python
# packages/harness/nion/agents/middlewares/my_middleware.py
...
2. Register in `packages/harness/nion/agents/lead_agent/agent.py`:
...
1. Update `packages/harness/nion/config/app_config.py` with new fields
...
Thank you for contributing to Nion!
