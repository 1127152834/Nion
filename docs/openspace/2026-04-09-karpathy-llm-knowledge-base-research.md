...
这轮研究之后，面向 Nion 的目标已经不该再表述成“做一个 AI 笔记本”或者“增强 Notebook”，而应该明确表述成：

**一个供 AI 助手和用户共同使用、共同维护的本地知识库。**

这个目标至少包含四个不可丢的要求：

1. **知识库是一等对象，不是聊天附件或普通笔记集合**
   它应该有明确的结构、索引、日志、图谱和治理规则。

2. **AI 助手必须把知识库当作主要工作底座**
   AI 不只是“读取一些检索片段来回答”，还应该优先读取知识库中的结构化页面，再必要时回退原始资料。

3. **AI 助手和用户要共同维护知识库**
   用户可以写、改、组织、判断；AI 可以 ingest、总结、交叉链接、生成 topic/concept/synthesis 页面、提出 lint 和更新建议。

4. **维护结果必须持续写回知识库**
   高价值回答、综合结论、比较分析、开放问题，不应只停留在 thread 里，而应沉淀成知识库对象。

一句话说，这不是“AI 读笔记”，而是“AI 和用户一起维护一个可持续演化的知识工作空间”。

和当前 Nion 的差距也因此非常具体：

- 现在的 notebook assistant 是 **围绕当前 note 的助手**
- 现在的 openviking notebook ingest 是 **把 note 投影成 chunks 供检索**
- 现在的 continuity path 是 **把检索结果注入 prompt**

这些都还不是“共同维护知识库”。共同维护知识库至少要求：
