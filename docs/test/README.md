...
| 模块 05 Settings / Config Center | 配置中心读取、校验、保存、运行时同步，影响模型、建议问题、工具、sandbox、daemon 等全局行为 | `frontend/src/components/workspace/settings/settings-dialog.tsx` 及各 section | `/api/config`、`/api/config/schema`、`/api/config/validate`、`/api/config/runtime-status` | `frontend/src/core/config-center/*`、`frontend/src/components/workspace/settings/*`、`backend/app/gateway/routers/config.py` | 高 | P0 |
| 模块 06 Notebook | 本地优先知识库，包含树、笔记编辑、历史、回收站、导入聊天内容、assist 改写 | `/workspace/notebook`、`/workspace/notebook/trash` | `/api/notebook/*` | `frontend/src/components/workspace/notebook/*`、`frontend/src/core/notebook/*`、`backend/app/gateway/routers/notebook.py`、`backend/packages/harness/nion/notebook/*` | 高 | P1 |
...
- 按页面/路由划分：`/workspace/chats`、`/workspace/cli-tools`、`/workspace/notebook`、`/workspace/automation`、`/workspace/agents`、`/workspace/bridge`、设置弹窗、tool policy 页面都对应独立入口。
