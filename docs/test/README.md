...
| 模块 06 Notebook | 本地优先知识与工作材料库，包含 inbox mixed feed、笔记编辑、历史、回收站、聊天沉淀、工作产物副本归档、assist 改写 | `/workspace/notebook`、`/workspace/notebook/trash` | `/api/notebook/*` | `frontend/src/components/workspace/notebook/*`、`frontend/src/core/notebook/*`、`backend/app/gateway/routers/notebook.py`、`backend/packages/harness/nion/notebook/*` | 高 | P1 |
...
- 按页面/路由划分：`/workspace/chats`、`/workspace/cli-tools`、`/workspace/notebook`、`/workspace/memory`、`/workspace/automation`、`/workspace/agents`、`/workspace/bridge`、设置弹窗、tool policy 页面都对应独立入口。
...
- 按状态流转闭环划分：例如 CLI tools 不只是列表展示，还包含 catalog -> install -> detect -> select -> 线程上下文注入；Notebook 包含收件箱 -> 编辑 -> 历史 -> 删除 -> 回收站恢复，以及显式 chat-to-note / asset archive 入口。
...
- 当前行为验收补充：
  - Memory / Soul 模块除了接口和页面冒烟，还必须跑一轮“行为验收题库”
  - 行为题库路径：
