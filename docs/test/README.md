...
| 模块 05 Settings / Config Center | 配置中心读取、校验、保存、运行时同步，影响模型、建议问题、工具、sandbox、daemon 等全局行为；M5 后只承担配置与轻量跳转，不再作为 Memory / Self-Maintenance 的主产品承载面 | `frontend/src/components/workspace/settings/settings-dialog.tsx` 及各 section | `/api/config`、`/api/config/schema`、`/api/config/validate`、`/api/config/runtime-status` | `frontend/src/core/config-center/*`、`frontend/src/components/workspace/settings/*`、`backend/app/gateway/routers/config.py` | 高 | P0 |
| 模块 06 Notebook | 本地优先知识库，作为 `Knowledge Base` 独立域，包含树、笔记编辑、历史、回收站、导入聊天内容、assist 改写 | `/workspace/notebook`、`/workspace/notebook/trash` | `/api/notebook/*` | `frontend/src/components/workspace/notebook/*`、`frontend/src/core/notebook/*`、`backend/app/gateway/routers/notebook.py`、`backend/packages/harness/nion/notebook/*` | 高 | P1 |
...
- 按页面/路由划分：`/workspace/chats`、`/workspace/cli-tools`、`/workspace/notebook`、`/workspace/memory`、`/workspace/self-maintenance`、`/workspace/projects`、`/workspace/automation`、`/workspace/agents`、`/workspace/bridge`、设置弹窗、tool policy 页面都对应独立入口。
...
- 按状态流转闭环划分：例如 CLI tools 不只是列表展示，还包含 catalog -> install -> detect -> select -> 线程上下文注入；Notebook 包含创建 -> 编辑 -> 历史 -> 删除 -> 回收站恢复。
...
- 前端以 contract test / node test 为主，重点覆盖 permission request、cli tools routes、notebook 组件 contract、memory/self-maintenance/product-surface contract、desktop thread client、settings 分区逻辑。
- 缺口主要在真实 UI 交互与跨模块 E2E；本批文档的价值就是把这些缺口转成可执行任务。
- M5 后新增必做 IA 冒烟：`/workspace/notebook`、`/workspace/memory`、`/workspace/self-maintenance`、`/workspace/projects` 的独立导航与心智分离验证。
