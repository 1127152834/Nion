...
| 模型与 provider 能力 | Codex streamed-output、Claude OAuth billing、vLLM/Ollama、subagent model override | 5 | `0948c7a4`, `43ef3691`, `dd30e609`, `133ffe71`, `ac04f270` | A | H | 2-4 | CFG | 部分同步 / 已等价吸收 / 明确不同步 | P1 | vLLM 已有；Codex/Claude provider 修复值得局部核对；Ollama/subagent override 不能按 `config.yaml` 路线直接拉。 |
...
| `0948c7a4` | preserve streamed Codex output when completed output is empty | `openai_codex_provider.py` | Codex streamed output 可能被空 completed output 覆盖 | NION 有 Codex provider 和 reasoning tests，但需核对该具体失败模式 | 未确认 | 部分同步 | provider 局部修复，需避免破坏本地 model registry | `architecture-source-review.md`; `architecture-nion-comparison.md`; `backend/packages/harness/nion/models/openai_codex_provider.py` |
