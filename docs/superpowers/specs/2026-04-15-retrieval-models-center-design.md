...
- 桌面端本地模型下载、导入、选择、probe 已恢复
- 只有当本地包同时包含 ONNX 权重、tokenizer 与 config 资产时，才能把它当成“已恢复本地语义索引执行”
- 在这种阶段下，产品必须明确：
  - 本地模型可下载、可选、可探测
  - 资产完整时，Memory 本地语义索引执行可用
  - 资产不完整时，必须明确报错或回退，而不是假装成功
