...
**Goal:** 以小步快跑但架构正确的方式，把 Claude Code 最有价值的工具面、运行时治理、skills/plugin/MCP 能力逐步融合进 Nion。

**Architecture:** 不追求表面对齐 Claude Code 的工具数量，而是优先补齐 Nion 在 coding-agent 场景下缺失的能力层。先做 coding-facing tools，再做工具执行治理，再做 skill/plugin/MCP 生态兼容，最后收拢到统一的运行时与产品体验层。
...
- Claude Code 关键参考：
  - `/Users/zhangtiancheng/Documents/项目/agent/claude-code/src/Tool.ts`
  - `/Users/zhangtiancheng/Documents/项目/agent/claude-code/src/tools.ts`
  ...
  - `/Users/zhangtiancheng/Documents/项目/agent/claude-code/src/tools/SkillTool/SkillTool.ts`
...
- 现有内置工具：
  - `/Users/zhangtiancheng/Documents/项目/agent/nion/backend/packages/harness/nion/tools/builtins/`
- lead agent prompt / subagent prompt：
  - `/Users/zhangtiancheng/Documents/项目/agent/nion/backend/packages/harness/nion/agents/lead_agent/prompt.py`
  - `/Users/zhangtiancheng/Documents/项目/agent/nion/backend/packages/harness/nion/subagents/`
- Guardrails / policy：
...
- MCP：
  ...
  - `/Users/zhangtiancheng/Documents/项目/agent/nion/backend/packages/harness/nion/mcp/tools.py`
- 前端消息与任务面：
  - `/Users/zhangtiancheng/Documents/项目/agent/nion/frontend/src/components/ai-elements/`
  - `/Users/zhangtiancheng/Documents/项目/agent/nion/frontend/src/components/workspace/`
...
- 计划与设计文档：
  ...
  - Create later as needed:
    - `/Users/zhangtiancheng/Documents/项目/agent/nion/docs/superpowers/specs/2026-04-01-tool-activity-layer-design.md`
...
Claude Code 的能力不应该按“抄 16 个 tool 名字”来复刻，而应该按下面 4 层来推进：

1. **Coding-facing tools**：先补能显著提升 coding agent 实战能力的工具面
2. **Tool runtime governance**：再补工具执行前后治理、权限、hook、摘要
3. **Execution primitives**：再做 SkillTool、AgentTool、Plan mode 的一等运行时语义
4. **Extension plane**：最后做 plugin 和 MCP 的深度兼容

这个顺序可以保证每一步都对 Nion 有独立价值，不会为了追求 Claude Code 同构而把系统打散。

---

## 阶段 1：补齐高价值 Coding Tools
...
1. **SkillTool**
   - 现状：Nion 有 skills 与 `read_skill`/`update_skill` 管理面，但没有模型一等调用 skill 的执行工具
   - 价值：打开 Claude Code skill 工作流兼容入口
2. **GlobTool**
...
- Skill 可以通过一等工具被调用，而不是靠 prompt 约定
- 文件编辑不再只依赖整文件重写和字符串替换
- LSP 能提供至少“诊断 + 跳转/符号信息”中的一部分

---

## 阶段 2：建立 Tool Activity Layer
...
因为它依赖第一阶段的新工具面，尤其是专用 Grep/Glob/FileEdit/LSP 的结果结构；先补工具，再做活动层，数据才稳定。
...
## 阶段 3：升级 Tool Runtime Governance
...
- 现有 GuardrailMiddleware 继续保留，但应降级为统一 tool pipeline 中的一层
...
因为 `SkillTool` 不只是一个工具，它会反过来塑造 prompt、tool contract、plugin 兼容方式。
...
- 业务逻辑上可以直接向 Claude Code 致敬，甚至做高度兼容
...
## 阶段 8：Prompt Runtime 与 Context Hygiene 收口

### 目标

这是把 Nion 从“有很多能力”收口成“像产品”的阶段。

### 要补的能力

1. system prompt section registry
2. static/dynamic boundary
3. tool result summary / transcript hygiene
4. compact / collapse / reminder retention
5. 会话恢复时保留 skill / tool summary / todo 等运行时状态

### 验收标准

- prompt 不是一大块拼接字符串
- 长会话稳定性明显提升
- tool activity / skills / todos 在压缩与恢复后仍然连贯
...
7. Tool Runtime Governance
8. AgentTool / specialist agents
9. Plugin 兼容层
10. MCP 深度融合
11. Prompt runtime 与 context hygiene

---

## 里程碑验收

### 里程碑 A：Coding Tool Surface 完整

- SkillTool / Glob / Grep / FileEdit / LSP 至少具备第一版可用实现
- Agent 主要代码探索和编辑不再依赖 shell 兜底

### 里程碑 B：Tool Activity 可视化

- 聊天流、子任务、diagnostics 共用统一工具活动语义
- 长任务不再只有原始工具噪音

### 里程碑 C：Runtime Contract 成型

- 工具执行、权限、hook、摘要、拒绝形成统一链路
...
- **先补 tools：** `SkillTool`、`GlobTool`、`GrepTool`、增强版 `FileEditTool`、`LSPTool`
- **补完后立刻进入：** `Tool Activity Layer`
- **再往后才值得投入：** `Tool Runtime Governance`、`AgentTool`、`Plugin`、`MCP`

这条路线不是最短的，但对 Nion 来说是价值最高、返工最少的。
