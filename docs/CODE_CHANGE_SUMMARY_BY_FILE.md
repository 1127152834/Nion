...
### 2. `backend/packages/harness/nion/agents/lead_agent/prompt.py`
...
### 4. `backend/packages/harness/nion/subagents/builtins/general_purpose.py`
...
@@ -1,12 +1,3 @@
-<citations>
-{"id": "cite-1", "title": "Nion GitHub Repository", "url": "https://github.com/huanxi/nion", "snippet": "..."}
-...（共 7 条 JSONL）
-</citations>
 # Nion Deep Research Report
 
 - **Research Date:** 2026-02-01
```

- 删除文件开头的 `<citations>...</citations>` 整块（9 行），正文从 `# Nion Deep Research Report` 开始。

---

### 26. `frontend/public/demo/threads/.../thread.json`

- **主要变更**：某条 `write_file` 的 `args.content` 中，将原来的「`<citations>...\n</citations>\n# Nion Deep Research Report\n\n...`」改为「`# Nion Deep Research Report\n\n...`」，即去掉 `<citations>...</citations>` 块，保留其后全文。
